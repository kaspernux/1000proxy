<script>
    // Avoid JsonException on malformed UTF-8 by base64-encoding the markdown first
    let exceptionAsMarkdown = (() => {
        try {
            // The vendor view passed a $markdown string; we guard and encode safely
            const b64 = '{{ base64_encode($markdown ?? ($exceptionAsMarkdown ?? "")) }}';
            const raw = atob(b64);
            // Ensure it's valid UTF-8 in JS context
